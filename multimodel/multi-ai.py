...
def generate_ebook(input_text):
    ...
    formatted_and_topics = format_and_topics_chain.run({"text": input_text})
    ...
    content = []
    for topic in topics:
        section_content = write_content_chain.run({"topic": topic})
    ...
    final_ebook = combine_ebook_chain.run({"content": full_content})
